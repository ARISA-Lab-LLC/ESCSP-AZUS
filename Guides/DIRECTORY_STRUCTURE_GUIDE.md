...
└── ESID#004/
    ...
    └── wav_hashes.csv                   ← OPTIONAL cache of each file's
                                           SHA-512 (+ md5) — see
                                           Resources/hash_raw_wavs.py.
                                           Not uploaded; not part of the ZIP.
```

> `wav_hashes.csv` only appears once `Resources/hash_raw_wavs.py` or a
> file-by-file upload has run. It is never uploaded to Zenodo and never enters
> the ZIP: `prepare_dataset.py` takes only `*.WAV`/`*.wav` and `CONFIG.TXT` out
> of a raw folder. Deleting it is always safe — it only costs a re-hash.
>
> Columns: `File Name, File Size (Bytes), Modified (epoch), SHA-512, MD5`. The
> `MD5` cell is empty unless `hash_raw_wavs.py --backfill-md5` or a file-by-file
> upload has filled it; a row without it is still fully valid for SHA-512.
